<template>
  <div>
    <el-button type="primary" icon="el-icon-plus" @click="showAdd" v-if="$hasBP('btn.Trademark.add')">添加</el-button>
    ...
    <el-table v-loading="loading" style="margin: 20px 0" :data="trademarks" border>
      ...
      <el-table-column label="操作">
        <template slot-scope="{row, $index}">
          <el-button type="warning" size="mini" icon="el-icon-edit" @click="showUpdate(row)"
    ...
    <el-dialog :title="form.id ? '修改品牌' : '添加品牌'" :visible.sync="isShowDialog">
      <el-form :model="form" :rules="rules" ref="ruleForm" style="width: 80%" label-width="100px">
